...
#tidyverse: A collection of several packages to help with data structures, plot generation and other functions.
```
...
waste_data_long <- waste_data_fct |> 
  pivot_longer(cols = food_organic_waste:yard_garden_green_waste, names_to = "waste_category", values_to = "percent") |> 
...
#For humans the data is better visualized in a barplot or wide format, where the categories can be condensed again
```
...
#better readability, but some columns are useless (no data)
```
